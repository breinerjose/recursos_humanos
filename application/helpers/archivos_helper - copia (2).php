 ...
 function listar_archivos_asap($carpeta){
    ...
    if(is_dir($carpeta)){
		...
		foreach($correo as $co){			
			...
			$sql2="select emaemp from vista_email_cliente  where nomemp=?";
			$res3=$ci->db->query($sql2,$dividir[0]);
			if($res3->num_rows()>0){
			...
			foreach($fi['emaemp'] as $co){
				...
				if(count($nuevo)>0){
					...
					$mensaje2 =  '<p>Buenos Dias</p>
								  <p>Cordial Saludo</p>
								  <p>En Nombre de Asap S.A.S Adjunto seguridad social correspondiente al mes '.$mes.' del a&ntilde;o 2017, dando cumplimiento con lo establecido en el Art. 13 
					...
					if(enviarcorreo($datosCorreo,$mes,'ASAP','enviarasap')){
					echo "Enviado ".$fi['archivo']."<br>";
						$enviado=array('nomemp'=>$empresa[0],'archivo'=>$fi['archivo']);	
	...
	foreach($final as $i=>$fi){
			...
			foreach($fi['emaemp'] as $co){
				...
				if(count($nuevo)>0){
					...
					$mensaje2 =  '<p>Buenos Dias</p>
								  <p>Cordial Saludo</p>
								  <p>En Nombre de Aseco S.A.S Adjunto seguridad social correspondiente al mes '.$mes.' del a&ntilde;o 2017, dando cumplimiento con lo establecido en el Art. 13 
								  del Decreto 4369 de 2006.</p>
								  <p>Agradezco la atencion prestada</p> 
								  <p>por favor confirmar recibido</p>
								  <p>Enviado mediante Sistema Automatizado</p>
					...
					if(enviarcorreo($datosCorreo,$mes,'ASECO','enviaraseco')){
					echo "Enviado ".$fi['archivo']."<br>";
						$enviado=array('nomemp'=>$empresa[0],'archivo'=>$fi['archivo']);	
						$resp3=$ci->db->insert('correo_enviado',$enviado);
						if($ci->db->affected_rows()>0){
...
function enviarcorreo($datosCorreo,$mes,$empresa,$carpeta) {
    $ci = & get_instance();
