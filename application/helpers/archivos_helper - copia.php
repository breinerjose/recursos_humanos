			...
			foreach($fi['emaemp'] as $co){
				...
				if(count($nuevo)>0){
					...
					$mensaje2 =  '<p>Buenos Dias</p>
								  <p>Cordial Saludo</p>
								  <p>En Nombre de Asap S.A.S Adjunto seguridad social correspondiente al mes Junio del a&ntilde;o 2017, dando cumplimiento con lo establecido en el Art. 13 
...
function enviarcorreo($datosCorreo) {
    $ci = & get_instance();
    ...
    try {
        ...
        $mail->SetFrom('info@asapaseco.com', 'ASECO - ASAP');
        ...
        $mail->AddAttachment('./enviarasap/' . $datosCorreo['adjunto']);
		$mail->AddAttachment('./enviarasap/ASAP PAGO  PLANILLA SEGURIDAD SOCIAL MES '.$fecha.'.pdf');
        $mail->Send();
        return true;
