...
$(document).ready(function() {
    $('.actualizar').button({
		...
		if(codigo!=''){
			if(!confirm("Esta seguro de Reabrir el Paz y Salvo?")) {return false;} 
              else {   
					 $.post('/pazysalvo/retirar_c/revertirPazysalvo',{'numero':codigo},function(resp){
						  if(resp == '1'){
...
<form id="buscar" name="buscar" method="post">
    ...
    <input type="text" id="numero" name="numero" class="txt required" placeholder="Digite #"/>
   ...
   <a href="#" class="actualizar">Reabrir Paz y Salvo</a>
   </div>
</form>