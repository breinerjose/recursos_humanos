...
<title>Cesasiones Laborales</title>
...
function dtl(){
		oTable = $('#cesasiones').dataTable( {
			"bProcessing": true,
			"bDestroy" : true,
			"bPaginate": true,
			"sScrollY" : '160px',
			"sPaginationType": "full_numbers",
			"sAjaxSource": "/pazysalvo/retirar_c/consultarCesasionlaboral/",
			"oLanguage": {"sUrl": "/recursos/js/datatables/media/language/espanol.txt"}
		});
	}	
$(document).ready(function(){
	dtl();
	$('#cesasiones').on('click','.add',function(){
		var codigo = $(this).attr('data-cod');
		window.parent.datoCesasion(codigo);
		});
...
<body>
<table id="cesasiones" cellpadding="0" cellspacing="0" class="display" width="100%">
<thead>
<tr>
