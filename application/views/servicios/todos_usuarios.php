...
function dtl(){
		oTable = $('#tabla_usuarios').dataTable( {
			...
			"sAjaxSource": "<?php echo base_url(); ?>servicios_c/obtener_usuarios/",
			"oLanguage": {"sUrl": "<?php echo base_url(); ?>recursos/js/datatables/media/language/espanol.txt"}
		});
	}	
$(document).ready(function(){
	dtl();
	$('#tabla_usuarios').on('click','.add',function(){
		var coduser = $(this).attr('data-cod');var nomuser = $(this).attr('data-nom');
		window.parent.datos_user(coduser,nomuser);
	});
...
<body>
<table id="tabla_usuarios" cellpadding="0" cellspacing="0" class="display" width="100%">
<thead>
<tr>
